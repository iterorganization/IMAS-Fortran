...
program test_shim_stamp_mismatch_no_artifact
  ...
  if (len_trim(fixture) == 0) error stop 'SCENARIO-FAILURE: missing stamp-mismatch fixture'

  call imas_open('imas:hdf5?path='//trim(fixture), OPEN_PULSE, context, open_status)
  if (open_status /= 0) error stop 'SCENARIO-FAILURE: stamp-mismatch-no-artifact open did not forward'

  call ids_get(context, 'equilibrium', equilibrium, get_status)
  call imas_close(context)

  if (get_status /= 0) error stop 'SCENARIO-FAILURE: stamp-mismatch-no-artifact read did not forward cleanly'
  if (al_get_skipped_count() /= 0) error stop 'SCENARIO-FAILURE: stamp-mismatch-no-artifact read logged a skipped path'
  if (.not. associated(equilibrium%time)) error stop 'SCENARIO-FAILURE: stamp-mismatch-no-artifact read reached no data'
  if (size(equilibrium%time) /= 2) error stop 'SCENARIO-FAILURE: stamp-mismatch-no-artifact read returned an unexpected time base'
  if (.not. associated(equilibrium%time_slice)) &
    error stop 'SCENARIO-FAILURE: stamp-mismatch-no-artifact read reached no time slices'
  ...
  if (equilibrium%time_slice(1)%global_quantities%beta_tor_norm > absent_threshold) then
    error stop 'SCENARIO-FAILURE: stamp-mismatch-no-artifact read populated a renamed field: conversion ran with no artifact'
  end if
